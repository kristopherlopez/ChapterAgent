"""SHAP-based explainability for the Australian Credit Approval Scorer."""
...
def compute_shap_values(
    ...
    *,
    ...
    Uses LinearExplainer for LogisticRegression (primary model),
    TreeExplainer for tree-based models, generic Explainer as fallback.

    For LinearExplainer, a separate background dataset should be provided
    when explaining a small number of samples (e.g. single predictions).
    """
    ...
    elif model_type in ("GradientBoostingClassifier", "RandomForestClassifier", "XGBClassifier"):
...
def top_contributors(
    ...
    *,
    ...
    Note: Features are anonymised (A1-A14), so SHAP explanations say
    "A9 contributed +0.18" — interpretable to the model but opaque to
    end users. The platform flags this as an interpretability warning.
    """
    ...
    for idx in indices:
        ...
        contributors.append({
            ...
            "direction": "supports_approval" if val > 0 else "supports_denial",
